...
{
    /// <summary>
    /// This script was just copied from the OpenAI Unity plugin, and slightly altered to fit Groq instead
    /// Changing the initial script could have caused issues in Git, so it was better to do it this way
    /// </summary>
    public class GroqApi
    {
        ...
        /// Made public so it can be overwritten dynamically (e.g., to use Groq's OpenAI-compatible API).
        /// </summary>
        private const string BASE_PATH = "https://api.groq.com/openai/v1";

        public GroqApi(string apiKey = null, string organization = null)
        {
            if (apiKey != null)
            {
